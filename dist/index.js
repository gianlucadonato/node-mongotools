...
var MTDropbox = class {
  ...
  mongoDumpUploadOnDropbox(options, dumpResult) {
    return new Promise((resolve, reject) => {
      ...
      getDropbox(options).then((dbx) => {
        fs3.readFile(dumpResult.fullFileName, (readFileError, contents) => {
          if (readFileError) {
...
function extractFilename(fullFileName) {
  return path2.basename(fullFileName);
}
function getDropbox(options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    isAccessTokenValid(currentAccessToken).then((result) => {
      ...
      refreshAccessToken(dropboxRefreshToken, dropboxAppKey, dropboxAppSecret).then((freshAccessToken2) => {
        options.freshAccessToken = freshAccessToken2;
        resolve(new Dropbox({ "accessToken": freshAccessToken2 }));
      }).catch((err) => reject(err));
    });
  });
}
...
var MTEncrypt = class {
  encrypt(source, destination, secretKey, removeSource = true) {
    return new Promise((resolve) => {
      ...
      const inStream = fs4.createReadStream(source);
      const encrypt = crypto.createCipheriv(algorithm, secretKey, iv);
      const outFileStream = fs4.createWriteStream(destination);
...
var MTCommand = class {
  ...
  filenameOnly(fullName) {
    return fullName ? fullName.substring(fullName.lastIndexOf(path3.sep) + 1, fullName.length) : "";
  }
  /**
   * help user on usage
   */
